...
onMounted(() => {
    ...
    axios.get(`/api/Session/cinema/${route.params.cinemaId}`)
        .then((res) => {
            if (res.data.status && res.data.status === '10000') {
                ...
                <div class="flex flex-row my-5 pt-1">
                    <div class="grid place-item-center basis-1/3 mx-10">
                        <el-space alignment="center" :fill="true">
                            <el-image style="width: 300px; height: 400px;" :src="cinema.cinemaImageUrl" fit="contain" />
                        </el-space>
                    </div>
                    <el-space direction="vertical" alignment="start" size="large" :fill="true">
                        ...
                        <div>
                            ...
                            <el-space wrap size="small" direction="horizontal">
                                <el-tag v-for="tag in cinema.feature.split(',')" :key="tag" size="small"
                                    color="rgba(90,0,90,.4)" effect="dark" disable-transitions>{{ tag }}
                                </el-tag>
                            </el-space>
                        </div>
                    </el-space>
